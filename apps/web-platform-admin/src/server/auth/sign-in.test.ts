 ...
 *
 * Covers the security-critical role gate (`checkUserHasPlatformRole`) and the
 * overall sign-in flow.  Issue #221.
 *
...
describe('signIn', () => {
  ...
  it('returns { ok: true } for a user with a platform_admin role', async () => {
    ...
    mockCreateServerClient.mockReturnValue(
      makeServiceClient([{ role: 'platform_admin' }]) as never
    );
  ...
  it('returns { ok: true } for a user with a platform_support role', async () => {
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL', 'https://test.supabase.co');
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_ANON_KEY', 'anon-key');
    vi.stubEnv('SUPABASE_SERVICE_ROLE_KEY', 'service-key');
    ...
    mockCreateAnonSsrServerClient.mockReturnValue(anonClient as never);
    mockCreateServerClient.mockReturnValue(
      makeServiceClient([{ role: 'platform_support' }]) as never
    );

    const result = await signIn('support@example.com', 'secret');
  ...
  it('returns { ok: false, error: "unauthorized_role" } when user has a gym role (not a platform role)', async () => {
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL', 'https://test.supabase.co');
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_ANON_KEY', 'anon-key');
    vi.stubEnv('SUPABASE_SERVICE_ROLE_KEY', 'service-key');
    ...
    mockCreateAnonSsrServerClient.mockReturnValue(anonClient as never);
    // User has a gym role — not allowed in platform admin
    mockCreateServerClient.mockReturnValue(makeServiceClient([{ role: 'gym_owner' }]) as never);

    const result = await signIn('gymowner@example.com', 'secret');
  ...
  it('returns { ok: false, error: "unauthorized_role" } when user has no roles at all', async () => {
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL', 'https://test.supabase.co');
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_ANON_KEY', 'anon-key');
    vi.stubEnv('SUPABASE_SERVICE_ROLE_KEY', 'service-key');
